...
import { 
  ...
  X, Link as LinkIcon, Target, BookOpen, Trash2, Menu, Circle, CheckCircle, Video
} from 'lucide-react';

interface BcsResource { id: string; chapter_id: string; title: string; resource_type: string; file_url: string; is_completed: boolean; }
interface BcsChapter { id: string; title: string; resources: BcsResource[]; }
...
export default function BcsWorkspace() {
  ...
  useEffect(() => { if (subjectId) fetchWorkspaceData(); }, [subjectId]);

  useEffect(() => {
    if (activeResource) fetchNotesForResource(activeResource.id);
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try { await supabase.from('bcs_subjects').update({ status: newStatus }).eq('id', subjectId); } 
    catch (error) { console.error(error); }
  };
  ...
  const toggleResourceCompletion = async (resId: string, currentStatus: boolean) => {
    ...
    const newChapters = chapters.map(ch => ({
      ...ch, resources: ch.resources.map(r => r.id === resId ? { ...r, is_completed: newStatus } : r)
    }));
    setChapters(newChapters);
    if (activeResource?.id === resId) setActiveResource({ ...activeResource, is_completed: newStatus });

    let total = 0; let completed = 0;
    newChapters.forEach(ch => { ch.resources.forEach(r => { total++; if (r.is_completed) completed++; }); });
    ...
    try {
      ...
      if (pct > 0 && pct < 100 && subject.status === 'pending') handleStatusChange('active');
  ...
  const handleAddChapter = async (e: React.FormEvent) => {
    ...
    } catch (error) { alert("Error adding chapter"); }
  };

  const handleAddResource = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!rTitle || !showResourceModal || !rUrl) return;
    setIsSavingResource(true);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      await supabase.from('bcs_resources').insert([{ chapter_id: showResourceModal, user_id: user?.id, title: rTitle, resource_type: rType, file_url: rUrl }]);
      setRTitle(''); setRUrl(''); setShowResourceModal(null); fetchWorkspaceData();
  ...
  const SidebarContent = () => (
    <>
      <div className="p-4 border-b border-[#E2E8F0] flex justify-between items-center bg-[#F8FAFC] sticky top-0 z-10">
        <h2 className="font-bold flex items-center gap-2 text-[#020F33]"><BookOpen size={18} style={{ color: subject?.icon_color }}/> Syllabus</h2>
        <button onClick={() => setShowChapterModal(true)} className="hover:text-white p-1.5 rounded-lg transition-colors" style={{ color: subject?.icon_color, backgroundColor: `${subject?.icon_color}15` }}>
          <Plus size={18} />
        </button>
      </div>
        ...
        {chapters.map(chap => (
          <div key={chap.id} className="mb-5">
            <div className="flex justify-between items-center mb-2 px-2 border-b border-[#E2E8F0] pb-2 group">
              ...
              <button onClick={() => setShowResourceModal(chap.id)} className="text-[#475569] bg-[#E2E8F0]/50 rounded-md p-1 transition-colors hover:bg-[#02C2D5]/20 hover:text-[#02C2D5]"><Plus size={16} /></button>
            </div>
            <div className="space-y-1">
              {chap.resources.map(resource => (
                <div key={resource.id} className={`flex items-start gap-2 p-2 rounded-xl transition-all ${activeResource?.id === resource.id ? 'bg-[#020F33] text-white shadow-md' : 'hover:bg-[#F8FAFC]'}`}>
                  <button onClick={() => toggleResourceCompletion(resource.id, resource.is_completed)} className={`mt-1 shrink-0 ${resource.is_completed ? 'text-[#A3D803]' : (activeResource?.id === resource.id ? 'text-slate-400' : 'text-[#CBD5E1] hover:text-[#02C2D5]')}`}>
                    {resource.is_completed ? <CheckCircle size={18} /> : <Circle size={18} />}
                  </button>
                  <button onClick={() => { setActiveResource(resource); setShowMobileSidebar(false); }} className="flex-1 text-left flex items-start gap-2">
                    {resource.resource_type === 'pdf' ? <File size={16} className="shrink-0 mt-0.5 opacity-70"/> : <Video size={16} className="shrink-0 mt-0.5 opacity-70"/>}
                    <span className={`text-sm font-medium leading-snug line-clamp-2 ${resource.is_completed && activeResource?.id !== resource.id ? 'line-through text-slate-400' : ''}`}>{resource.title}</span>
                  </button>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="h-screen flex flex-col bg-[#F8FAFC] text-[#020F33] overflow-hidden relative">
      ...
      {showChapterModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-lg">Create New Chapter</h3><button onClick={() => setShowChapterModal(false)} className="text-slate-400 hover:text-red-500"><X size={20}/></button></div>
            <form onSubmit={handleAddChapter} className="space-y-4">
              <input type="text" value={chTitle} onChange={(e)=>setChTitle(e.target.value)} placeholder="e.g. Chapter 1: Basic" className="w-full border border-[#E2E8F0] focus:border-[#02C2D5] rounded-xl p-3 outline-none" required/>
              <button type="submit" className="w-full bg-[#020F33] text-white font-bold py-3 rounded-xl hover:bg-[#02C2D5] transition-colors">Create Chapter</button>
            </form>
          </div>
        </div>
      )}
      ...
      {showResourceModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-5"><h3 className="font-bold text-lg">Add Resource</h3><button onClick={() => setShowResourceModal(null)} className="text-slate-400 hover:text-red-500"><X size={20}/></button></div>
            <form onSubmit={handleAddResource} className="space-y-4">
              <input type="text" value={rTitle} onChange={(e)=>setRTitle(e.target.value)} placeholder="Resource Title" className="w-full border border-[#E2E8F0] focus:border-[#02C2D5] rounded-xl p-3 outline-none" required/>
              <select value={rType} onChange={(e)=>setRType(e.target.value)} className="w-full border border-[#E2E8F0] focus:border-[#02C2D5] rounded-xl p-3 outline-none"><option value="youtube">YouTube Video</option><option value="video">Direct Video URL</option><option value="pdf">PDF Link</option></select>
              <input type="url" value={rUrl} onChange={(e)=>setRUrl(e.target.value)} placeholder="Link URL..." className="w-full border border-[#E2E8F0] focus:border-[#02C2D5] rounded-xl p-3 outline-none" required/>
              <button type="submit" disabled={isSavingResource} className="w-full bg-[#020F33] text-white font-bold py-3 rounded-xl hover:bg-[#02C2D5] hover:text-[#020F33] transition-colors disabled:opacity-50">{isSavingResource ? 'Saving...' : 'Save Resource'}</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleCreateNote} className="space-y-4">
              <input type="text" value={nTitle} onChange={(e)=>setNTitle(e.target.value)} placeholder="Note Title (e.g. Grammar Rules)" className="w-full border border-[#E2E8F0] focus:border-[#02C2D5] rounded-xl p-3 outline-none" required/>
              <button type="submit" className="w-full bg-[#020F33] text-white font-bold py-3 rounded-xl hover:bg-[#02C2D5] hover:text-[#020F33] transition-colors">Create Note</button>
            </form>
          </div>
        </div>
      )}
      ...
      <header className="bg-white border-b border-[#E2E8F0] px-3 md:px-6 py-3 flex flex-col md:flex-row md:items-center justify-between shrink-0 z-10 shadow-sm gap-3">
        <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
          <Link to="/bcs" className="p-2 bg-[#F8FAFC] hover:bg-[#E2E8F0] rounded-xl transition-colors shrink-0"><ArrowLeft size={18} className="text-[#475569]" /></Link>
          ...
          <div className="min-w-0 flex-1">
            <h1 className="font-bold text-sm md:text-lg leading-tight flex items-center gap-2 truncate"><span className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full shrink-0" style={{ backgroundColor: subject?.icon_color }}></span><span className="truncate">{subject?.title}</span></h1>
            <div className="flex items-center gap-2 mt-1 max-w-[200px]"><div className="flex-1 h-1.5 bg-[#F8FAFC] rounded-full overflow-hidden border border-[#E2E8F0]"><div className="h-full transition-all duration-500" style={{ width: `${subject?.progress_pct || 0}%`, backgroundColor: subject?.icon_color || '#02C2D5' }}></div></div><span className="text-[10px] font-black text-[#475569]">{subject?.progress_pct || 0}%</span></div>
          </div>
        </div>
        
        <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar shrink-0 ml-10 md:ml-2">
          <select value={subject?.status || 'pending'} onChange={(e) => handleStatusChange(e.target.value)} className={`text-xs md:text-sm font-bold px-3 py-1.5 md:py-2 rounded-lg border focus:outline-none transition-colors shrink-0 ${subject?.status === 'active' ? 'bg-[#02C2D5]/10 text-[#02C2D5] border-[#02C2D5]/20' : subject?.status === 'completed' ? 'bg-[#A3D803]/10 text-[#719900] border-[#A3D803]/20' : subject?.status === 'watch_later' ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-[#F8FAFC] text-[#475569] border-[#E2E8F0]'}`}>
            <option value="pending">⏳ Pending</option><option value="active">🔥 Active (Running)</option><option value="watch_later">📌 Watch Later</option><option value="completed">✅ Completed</option>
          </select>
        ...
        <main className={`flex-1 flex overflow-hidden bg-[#F8FAFC] p-2 md:p-4 gap-2 md:gap-4 ${viewMode === 'split' ? 'flex-col lg:flex-row' : 'flex-col'}`}>
          {(viewMode === 'split' || viewMode === 'media') && (
            <div className={`flex flex-col bg-black rounded-2xl overflow-hidden shadow-lg border border-[#E2E8F0] shrink-0 lg:shrink relative ${viewMode === 'split' ? 'h-2/5 lg:h-full lg:w-1/2' : 'h-full w-full'}`}>
              {activeResource && (
                <div className="absolute top-4 right-4 z-10"><button onClick={() => toggleResourceCompletion(activeResource.id, activeResource.is_completed)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold shadow-lg transition-all backdrop-blur-md border ${activeResource.is_completed ? 'bg-[#A3D803]/20 text-[#A3D803] border-[#A3D803]' : 'bg-black/50 text-white border-white/20 hover:bg-black/70'}`}>{activeResource.is_completed ? <CheckCircle2 size={14}/> : <Circle size={14}/>} {activeResource.is_completed ? 'Completed' : 'Mark as Done'}</button></div>
              )}
          ...
          {(viewMode === 'split' || viewMode === 'notes') && (
            <div className={`flex flex-col bg-white rounded-2xl overflow-hidden shadow-lg border border-[#E2E8F0] ${viewMode === 'split' ? 'h-3/5 lg:h-full lg:w-1/2' : 'h-full w-full'}`}>
              <div className="p-3 md:p-4 border-b border-[#E2E8F0] flex justify-between items-center bg-[#F8FAFC]">
                <h3 className="font-bold flex items-center gap-2 text-[#020F33] text-sm md:text-base truncate pr-2"><FileText size={16} className="text-[#02C2D5] shrink-0" /> <span className="truncate">{activeNote ? activeNote.title : 'Notes Editor'}</span></h3>
                {activeNote && <button onClick={handleSaveNote} disabled={isSavingNote} className="px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-xs md:text-sm font-bold flex items-center gap-2 bg-[#020F33] hover:bg-[#02C2D5] text-white hover:text-[#020F33] transition-colors shrink-0">{isSavingNote ? <Loader2 size={14} className="animate-spin" /> : <><Save size={14} /> <span className="hidden sm:inline">Save</span></>}</button>}
              </div>
              ...
              <div className="bg-[#F8FAFC] border-t border-[#E2E8F0] p-2 flex items-center gap-2 overflow-x-auto hide-scrollbar">
                <button onClick={() => setShowNoteModal(true)} disabled={!activeResource} className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 md:px-4 md:py-2 bg-[#020F33] text-white text-xs font-bold rounded-xl hover:bg-[#02C2D5] hover:text-[#020F33] transition-colors disabled:opacity-50"><Plus size={14} /> New File</button>
                {notesList.map(note => (<div key={note.id} onClick={() => { setActiveNote(note); setNoteContent(note.content); }} className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 md:py-2 border rounded-xl text-xs font-bold transition-all cursor-pointer group ${activeNote?.id === note.id ? 'bg-[#02C2D5]/10 border-[#02C2D5] text-[#020F33]' : 'bg-white border-[#E2E8F0] text-[#475569]'}`}><FileText size={12} className={activeNote?.id === note.id ? 'text-[#02C2D5]' : 'text-[#94A3B8]'} /> {note.title}</div>))}
              </div>
            </div>
          )}
        </main>
