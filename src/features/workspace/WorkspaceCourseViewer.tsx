...
export default function WorkspaceCourseViewer({ 
  ...
  const handleAddModule = async (e: React.FormEvent) => {
    ...
    const newModule: Module = { id: `mod-${Date.now()}`, title: newModuleTitle, contents: [] };
    const updatedModules = [...modules, newModule];

    try {
      const updatedCourseData = { ...courseData.content_data, modules: updatedModules };
      const { error } = await workspaceSupabase.from('shared_contents').update({ content_data: updatedCourseData }).eq('id', courseData.id);
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="p-3 pb-24 overflow-y-auto flex-1">
        {modules.length === 0 ? (
                ...
                {mod.contents?.map((content, cIdx) => (
                  <div key={content.id || cIdx} className={`flex items-start gap-2 p-2 rounded-xl transition-all ${activeContent?.title === content.title ? 'bg-[#020F33] text-white shadow-md' : 'hover:bg-[#F8FAFC]'}`}>
                    ...
                    <button onClick={() => { setActiveContent(content); setShowMobileSidebar(false); }} className="flex-1 text-left flex items-start gap-2">
                      ...
                      <span className={`text-sm font-medium leading-snug line-clamp-2 ${content.is_completed && activeContent?.title !== content.title ? 'line-through text-slate-400' : ''}`}>{content.title}</span>
                    </button>
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="h-screen flex flex-col bg-[#F8FAFC] text-[#020F33] overflow-hidden fixed inset-0 z-[100]">
      ...
      {isAddModuleOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#020F33]/60 backdrop-blur-sm" onClick={() => setIsAddModuleOpen(false)}></div>
          <div className="bg-white rounded-2xl w-full max-w-md p-6 relative z-10 shadow-2xl animate-in fade-in zoom-in">
            <div className="flex justify-between items-center mb-5">
      ...
      <header className="bg-white border-b border-[#E2E8F0] px-3 md:px-6 py-3 flex flex-col md:flex-row md:items-center justify-between shrink-0 shadow-sm gap-3">
        ...
        <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar shrink-0 ml-10 md:ml-2">
          <div className="text-xs md:text-sm font-bold px-3 py-1.5 md:py-2 rounded-lg border bg-purple-50 text-purple-600 border-purple-200">🔥 Active (Learning)</div>
            ...
            <div className={`flex flex-col bg-black rounded-2xl overflow-hidden shadow-lg border border-[#E2E8F0] shrink-0 lg:shrink relative ${viewMode === 'split' ? 'h-2/5 lg:h-full lg:w-1/2' : 'h-full w-full'}`}>
              {activeContent && (
                <div className="absolute top-4 right-4 z-10">
                  <button onClick={() => toggleContentCompletion(activeContent.id || activeContent.title, activeContent.is_completed)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold shadow-lg transition-all backdrop-blur-md border ${activeContent.is_completed ? 'bg-[#A3D803]/20 text-[#A3D803] border-[#A3D803]' : 'bg-black/50 text-white border-white/20 hover:bg-black/70'}`}>
                    {activeContent.is_completed ? <CheckCircle2 size={14}/> : <Circle size={14}/>} {activeContent.is_completed ? 'Completed' : 'Mark as Done'}
                  </button>
                </div>
              )}
