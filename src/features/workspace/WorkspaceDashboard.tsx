...
export default function WorkspaceDashboard() {
  ...
  const [isUpdatingProfile, setIsUpdatingProfile] = useState(false); // <--- New State Added
  ...
  // 👇 UPDATED: checkAuthAndFetchData with .maybeSingle()
  const checkAuthAndFetchData = async () => {
    ...
    // Fetch Profile Nickname (Fixed: using maybeSingle instead of single)
  ...
  // 👇 NEW: handleUpdateProfile function
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0D0E0F] text-slate-900 dark:text-[#F5F5F5] font-sans pb-24 transition-colors duration-300 selection:bg-[#FF9D2E]/30 relative">
      ...
      <header className="sticky top-0 bg-white/80 dark:bg-[#141516]/80 backdrop-blur-xl border-b border-slate-200 dark:border-[#292B2E] z-40 px-4 md:px-8 py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="hidden md:flex items-center gap-3 pl-4 border-l border-slate-200 dark:border-[#292B2E]">
            ...
            <div className="w-10 h-10 bg-[#FF9D2E]/10 rounded-full flex items-center justify-center border border-[#FF9D2E]/30 cursor-pointer" onClick={() => { setActiveTab('profile'); setSelectedGroup(null); }}>
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-8 mt-2">
        ...
        {/* PROFILE TAB (NEW UI FOR PROFILE UPDATES) */}
        {!selectedGroup && activeTab === 'profile' && (
          <div className="animate-fade-in space-y-8">
            <div className="bg-white dark:bg-[#18191A] p-8 md:p-12 rounded-3xl border border-slate-200 dark:border-[#292B2E] shadow-sm flex flex-col items-center max-w-2xl mx-auto mt-8">
              <div className="w-24 h-24 bg-[#FF9D2E]/10 rounded-full flex items-center justify-center border-2 border-[#FF9D2E]/30 mb-6">
                <User size={48} className="text-[#FF9D2E]" />
              </div>
              <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2">Edit Your Profile</h2>
              <p className="text-slate-500 dark:text-[#A3A5A8] text-center mb-8">Update your nickname. This name will appear in group chats and on the dashboard.</p>
              
              <div className="w-full space-y-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Nickname</label>
                  <input 
                    type="text" 
                    value={profileName} 
                    onChange={(e) => setProfileName(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-[#141516] border border-slate-200 dark:border-[#292B2E] focus:border-[#FF9D2E] rounded-xl p-4 outline-none transition-all text-slate-900 dark:text-white"
                ...
                <div className="pt-4">
                  <button 
                    ...
                    className="w-full bg-[#FF9D2E] text-slate-900 px-6 py-4 rounded-xl font-bold hover:bg-[#FFAA3D] disabled:opacity-50 transition-colors flex justify-center items-center gap-2"
                  >
                    {isUpdatingProfile ? (
                      <><div className="w-5 h-5 border-2 border-slate-900 border-t-transparent rounded-full animate-spin"></div> Saving...</>
                    ) : (
                      'Save Changes'
                    )}
                  </button>
      ...
      <nav className="md:hidden fixed bottom-0 w-full bg-white/90 dark:bg-[#141516]/90 backdrop-blur-xl border-t border-slate-200 dark:border-[#292B2E] pb-safe z-40">
        <div className="flex justify-around items-center h-16">
          ...
          <button onClick={() => { setActiveTab('profile'); setSelectedGroup(null); }} className={`flex flex-col items-center justify-center w-full space-y-1 ${activeTab === 'profile' ? 'text-[#FF9D2E]' : 'text-slate-400 dark:text-[#707277]'}`}>
