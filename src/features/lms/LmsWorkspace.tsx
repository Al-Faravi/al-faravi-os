...
import { 
  ...
  X, Link as LinkIcon, Target, Layout, Trash2, Menu, Circle, CheckCircle
} from 'lucide-react';
...
export default function LmsWorkspace() {
  ...
  useEffect(() => {
    if (courseId) fetchWorkspaceData();
  }, [courseId]);

  useEffect(() => {
    if (activeContent) {
      fetchNotesForContent(activeContent.id);
    } else {
    ...
    try {
      ...
      if (modulesData && modulesData.length > 0) {
        ...
        const structuredModules = modulesData.map(mod => ({
          ...mod,
          contents: (contentsData || []).filter(c => c.module_id === mod.id)
        }));
  ...
  const toggleContentCompletion = async (contentId: string, currentStatus: boolean) => {
    ...
    try {
      ...
      if (pct > 0 && pct < 100 && course.status === 'pending') {
  ...
  const handleAddContent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cTitle || !showContentModal) return;
    setIsSavingResource(true);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      let finalUrl = cUrl;
      if (cFile) {
        const filePath = `${user?.id}/${Math.random()}.${cFile.name.split('.').pop()}`;
        await supabase.storage.from('lms-resources').upload(filePath, cFile);
        const { data: { publicUrl } } = supabase.storage.from('lms-resources').getPublicUrl(filePath);
        finalUrl = publicUrl;
      }
      await supabase.from('lms_contents').insert([{ module_id: showContentModal, user_id: user?.id, title: cTitle, content_type: cType, file_path_or_url: finalUrl }]);
      setCTitle(''); setCUrl(''); setCFile(null); setShowContentModal(null); fetchWorkspaceData();
  ...
  const SidebarContent = () => (
    <>
      <div className="p-4 border-b border-[#E2E8F0] flex justify-between items-center bg-[#F8FAFC] sticky top-0 z-10">
        ...
        <button onClick={() => setShowModuleModal(true)} className="hover:text-[#020F33] p-1.5 rounded-lg transition-colors text-purple-600 bg-purple-50">
          <Plus size={18} />
        </button>
      </div>
        ...
        {modules.map(mod => (
          <div key={mod.id} className="mb-5">
            <div className="flex justify-between items-center mb-2 px-2 border-b border-[#E2E8F0] pb-2">
              ...
              <button onClick={() => setShowContentModal(mod.id)} className="text-[#A3D803] hover:text-[#020F33] bg-[#F8FAFC] rounded-md p-1"><Plus size={16} /></button>
            </div>
            <div className="space-y-1">
              {mod.contents.map(content => (
                <div key={content.id} className={`flex items-start gap-2 p-2 rounded-xl transition-all ${activeContent?.id === content.id ? 'bg-[#020F33] text-white shadow-md' : 'hover:bg-[#F8FAFC]'}`}>
                  {/* Mark as Done Toggle */}
                  <button 
                    onClick={() => toggleContentCompletion(content.id, content.is_completed)}
                    className={`mt-1 shrink-0 ${content.is_completed ? 'text-[#A3D803]' : (activeContent?.id === content.id ? 'text-slate-400' : 'text-[#CBD5E1] hover:text-purple-500')}`}
                  >
                  ...
                  <button onClick={() => { setActiveContent(content); setShowMobileSidebar(false); }} className="flex-1 text-left">
                    <span className={`text-sm font-medium leading-snug line-clamp-2 ${content.is_completed && activeContent?.id !== content.id ? 'line-through text-slate-400' : ''}`}>
                      {content.title}
                    </span>
                  </button>
                </div>
    ...
    <div className="h-screen flex flex-col bg-[#F8FAFC] text-[#020F33] overflow-hidden relative">
      ...
      <header className="bg-white border-b border-[#E2E8F0] px-3 md:px-6 py-3 flex flex-col md:flex-row md:items-center justify-between shrink-0 z-10 shadow-sm gap-3">
        <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
          <Link to="/lms" className="p-2 bg-[#F8FAFC] hover:bg-[#E2E8F0] rounded-xl transition-colors shrink-0">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="font-bold text-sm md:text-lg leading-tight flex items-center gap-2 truncate">
              <span className="truncate">{course?.title || 'Workspace'}</span>
            </h1>
        ...
        <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar shrink-0 ml-10 md:ml-2">
          ...
          <select 
            ...
            className={`text-xs md:text-sm font-bold px-3 py-1.5 md:py-2 rounded-lg border focus:outline-none transition-colors shrink-0
              ${course?.status === 'active' ? 'bg-purple-50 text-purple-600 border-purple-200' : 
                course?.status === 'completed' ? 'bg-[#A3D803]/10 text-[#719900] border-[#A3D803]/20' : 
                course?.status === 'watch_later' ? 'bg-amber-50 text-amber-600 border-amber-200' : 
            ...
            <div className={`flex flex-col bg-black rounded-2xl overflow-hidden shadow-lg border border-[#E2E8F0] shrink-0 lg:shrink relative ${viewMode === 'split' ? 'h-2/5 lg:h-full lg:w-1/2' : 'h-full w-full'}`}>
              ...
              {activeContent && (
                <div className="absolute top-4 right-4 z-10">
                  <button 
                    onClick={() => toggleContentCompletion(activeContent.id, activeContent.is_completed)}
          ...
          {(viewMode === 'split' || viewMode === 'notes') && (
            <div className={`flex flex-col bg-white rounded-2xl overflow-hidden shadow-lg border border-[#E2E8F0] ${viewMode === 'split' ? 'h-3/5 lg:h-full lg:w-1/2' : 'h-full w-full'}`}>
              ...
              <div className="bg-[#F8FAFC] border-t border-[#E2E8F0] p-2 flex items-center gap-2 overflow-x-auto hide-scrollbar">
                <button onClick={() => setShowNoteModal(true)} disabled={!activeContent} className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 md:px-4 md:py-2 bg-[#020F33] text-white text-xs font-bold rounded-xl hover:bg-purple-500 transition-colors disabled:opacity-50">
                ...
                {notesList.map(note => (
                  <div key={note.id} onClick={() => { setActiveNote(note); setNoteContent(note.content); }} className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 md:py-2 border rounded-xl text-xs font-bold transition-all cursor-pointer group ${activeNote?.id === note.id ? 'bg-purple-100 border-purple-400 text-[#020F33]' : 'bg-white border-[#E2E8F0] text-[#475569]'}`}>
                    <FileText size={12} className={activeNote?.id === note.id ? 'text-purple-600' : 'text-[#94A3B8]'} /> {note.title}
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Modals are kept hidden here for brevity */}
    </div>
  );
}